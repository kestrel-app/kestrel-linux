...
impl Alert {
    ...
    pub fn line(&self) -> String {
        if self.headline.is_empty() || self.headline == self.event {
            self.event.clone()
        } else {
            format!("{}  ·  {}", self.event, self.headline)
        }
    }
}
...
impl Model {
    ...
    pub fn alerts_line(&self) -> String {
        ...
        for alert in &self.alerts {
            ...
            out.push_str(&line);
            if !alert.ends.is_empty() {
                out.push_str("  ·  until ");
                out.push_str(&alert.ends);
            }
        }
        out
    }
