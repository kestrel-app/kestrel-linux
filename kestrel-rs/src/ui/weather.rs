...
impl WeatherView {
    ...
    pub fn bar(&self, ui: &mut egui::Ui, rect: Rect, prefs: &Preferences) {
        ...
        self.bar_middle(&painter, rect, middle_x, middle_right, h);
    ...
    /// them.
    fn bar_middle(&self, painter: &egui::Painter, rect: Rect, x: f32, edge: f32, h: f32) {
        ...
        self.bar_outlook(painter, rect, x, outlook_y, width, outlook_size);
    }
    ...
    /// show text that does not fit. Here it is ellipsised and the whole of it is
    /// on hover, which is what a pointer is for — and the pane below has every
    /// alert in full either way.
    fn bar_outlook(
        &self,
        painter: &egui::Painter,
        rect: Rect,
        ...
        if !self.model.alerts.is_empty() {
            let text = self.model.alerts_line();
            if !text.is_empty() {
    ...
    fn pane_alerts(&self, ui: &mut egui::Ui, prefs: &Preferences) {
        ...
        for (index, alert) in self.model.alerts.iter().take(shown).enumerate() {
            ...
            let mut detail = alert.headline.clone();
...
mod tests {
    ...
    fn the_alert_stack_is_bounded_and_says_so() {
        ...
        assert!(model.alerts_line().contains("Watch 6"));
    }
