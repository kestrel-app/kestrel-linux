...
DEFAULTS = {
    ...
    "allow_patterns": [],
    # The person who wrote this. What gets published is published by a project
    # or a company; whoever happened to have the shell open is not part of it,
    # and a name in a path is a home directory somebody can go looking for.
    "personal_names": [
        r"tschooley",  # noqa: public
        r"Tyler\s+Schooley",  # noqa: public
        r"\btyler\b",  # noqa: public
    ],
    ...
    "allow_unwanted": [],
    # Files that exist for us and never reach the public repository: the
    # publish script drops them from the tree it builds before it writes the
    # commit. They are not scanned, because what they say is not published.
    # The exclude list in publish-public.sh must say the same thing.
    "unpublished": [".gitignore", ".gitlab-ci.yml", "docs/internal.md"],
}
...
def build_checks(config: dict) -> list[tuple[str, re.Pattern[str], str]]:
    ...
    personal = "|".join(f"(?:{name})" for name in config["personal_names"]) or r"(?!)"
    ...
    return [
        ...
        (
            "personal name",
            re.compile(rf"(?:{personal})", re.I),
            "names the person who wrote this; publish under the project or "
            "the company instead",
        ),
    ]
...
def scan_worktree(root: Path, config, checks, allow) -> list[str]:
    ...
    for name in git(root, "ls-files").splitlines():
        if not name:
            continue
        if name in config["unpublished"]:
            continue  # dropped before the tree is published
