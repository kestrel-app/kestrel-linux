...
Reolink is what it does best and what it is tested against. Frigate, ZoneMinder,
QNAP QVR, UniFi Protect and anything speaking **ONVIF** are supported through the
same interface — see the note on those under *Notes and limitations*.
...
Press **+** in the Cameras sidebar, then **Find on network** — Kestrel looks for
cameras and NVRs on the networks this machine is attached to and lists what it
finds, with what each one turned out to be. Pick one and the address, port and
system are filled in; type the username and password and save.

Failing that, enter the address by hand. **Identify this system** asks it what it
is running, and **Test connection** confirms the credentials before saving — it
reports the model, firmware and channel list it found.
...
- **Other systems** — Frigate, ZoneMinder, QNAP QVR, UniFi Protect and ONVIF
  sit behind the same interface as Reolink, so a wall can mix them. Each
  reports what it can actually do rather than claiming everything: a system
  without playback says so instead of showing an empty calendar, and a camera
  that cannot move shows no PTZ pad. ONVIF is the general case — most cameras
  answer it, whoever made them — and it is tried last, so a Reolink NVR that
  also speaks ONVIF is still treated as a Reolink and keeps playback,
  floodlights and detections.
- **Find on network** — discovery for all of the above. ONVIF devices are asked
  to announce themselves; the systems that do not announce anything are found by
  looking across the networks this machine is attached to, on the ports these
  systems use. Bounded on purpose: directly attached networks only, nothing
  wider than a `/22`, and only those ports.
...
- **Only Reolink has been run against real hardware.** Frigate, ZoneMinder, QNAP
  QVR, UniFi Protect and ONVIF are written from their published APIs and the
  ONVIF standard, and none has ever been pointed at the real thing — there was
  none to point it at. They compile and their pure logic is tested;
  `kestrel-rs/docs/untested.md` says exactly what is known and what is
  guessed for each, and `kestrel-rs/tools/onvif-stub.py` is a stand-in ONVIF
  device the connect path is verified against.
- **Discovery finds what is reachable, not everything that exists.** A device on
  a network this machine is not attached to is only found if it answers the
  ONVIF announcement, and a sweep covers IPv4 only. Anything not found still
